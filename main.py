                ...
                if event.message.peer_id < 2000000000:
                    if not user.is_reg(user_id):
                        ...
                        if user.get_registration(user_id)[0] == 1:
                            ...
                            try:
                                await api.login(
                                                str(user.get_login(user_id)),  # Логин
                                                str(user.get_password(user_id)),  # Пароль
                                                str(user.get_name_of_school(user_id)))  # Название школы
                                vk.messages.send(user_id=user_id,
                                                 random_id=get_random_id(),
